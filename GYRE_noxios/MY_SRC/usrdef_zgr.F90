...
CONTAINS             
   ...
   SUBROUTINE usr_def_zgr( ld_zco  , ld_zps  , ld_sco  , ld_isfcav,    &   ! type of vertical coordinate
      ...
      !
      IF(lwp) WRITE(numout,*)
      IF(lwp) WRITE(numout,*) 'usr_def_zgr : GYRE configuration (z-coordinate closed box ocean without cavities)'
      IF(lwp) WRITE(numout,*) '~~~~~~~~~~~'
      !
      !
   ...
   SUBROUTINE zgr_z1d( pdept_1d, pdepw_1d, pe3t_1d , pe3w_1d )   ! 1D reference vertical coordinate
      ...
      IF(lwp) THEN                         ! Parameter print
         WRITE(numout,*)
         WRITE(numout,*) '    zgr_z1d : Reading dz in file dz.dat '
         WRITE(numout,*) '    ~~~~~~~'
      ENDIF

      open (unit=2212, file='dz.dat', status='old', action='read')
      read(2212,*) pe3t_1d (:)
      close(2212)

      pe3w_1d (1) = pe3t_1d (1)
      DO jk = 2, jpk
        pe3w_1d (jk) = 0.5_wp*(pe3t_1d (jk-1) + pe3t_1d (jk))
      END DO
      !
      !                  compute depths from SUM(e3) 
      CALL e3_to_depth( pe3t_1d, pe3w_1d, pdept_1d, pdepw_1d )

      !                       ! e3t and e3w from depth: needed??
