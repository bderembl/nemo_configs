...
CONTAINS
      ...
      DO jk = 1, jpkm1                                 ! Horizontal slab
         ...
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vector opt.
               ...
               zua = zua*(1. - rn_jamart) + zua*rn_jamart &
                  & * 4./MAX( 1., vmask(ji, jj, jk) + vmask(ji+1, jj, jk) + vmask(ji, jj-1, jk) + vmask(ji+1, jj-1, jk) )
               zva = zva*(1. - rn_jamart) + zva*rn_jamart &
                  & * 4./MAX( 1., umask(ji-1, jj+1, jk) + umask(ji, jj+1, jk) + umask(ji-1, jj, jk) + umask(ji, jj, jk) )
   ...
   SUBROUTINE dyn_vor_init
      ...
      NAMELIST/namdyn_vor/ ln_dynvor_ens, ln_dynvor_ene, ln_dynvor_enT, ln_dynvor_eeT,   &
         &                 ln_dynvor_een, nn_een_e3f   , ln_dynvor_mix, ln_dynvor_msk, rn_jamart
      !!----------------------------------------------------------------------
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namdyn_vor in configuration namelist', lwp )
      ...
      IF(lwp) THEN                    ! Namelist print
         ...
         WRITE(numout,*) '      jamart correction                              rn_jamart     = ', rn_jamart
      ENDIF
