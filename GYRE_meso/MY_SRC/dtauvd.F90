MODULE dtauvd
   ...
   CHARACTER(LEN=200) , PUBLIC ::   cn_resto_uv2    !: name of netcdf file containing restoration coefficient field

   TYPE(FLD), ALLOCATABLE, DIMENSION(:) ::   sf_uvd   ! structure for input U & V current (file information and data)
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::  resto_uv2  !: restoring coeff. on U & V current
...
CONTAINS
   ...
   SUBROUTINE dta_uvd_init( ld_dyndmp )
      ...
      !!
      NAMELIST/namc1d_uvd/ ln_uvd_init, ln_uvd_dyndmp, cn_dir, cn_resto_uv2, sn_ucur, sn_vcur
      !!----------------------------------------------------------------------
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namc1d_uvd in configuration namelist', lwp )
      ...
      IF(lwp) THEN                  ! control print
         ...
         WRITE(numout,*) '         Damping file name               cn_resto_uv = ', cn_resto_uv2
         WRITE(numout,*)
      ...
      IF(  ln_uvd_init .OR. ln_uvd_dyndmp  ) THEN
        ...
        ALLOCATE( resto_uv2(jpi,jpj,jpk), STAT=ierr0)
         IF( ierr0 > 0 ) THEN
         ...
         CALL iom_open ( cn_resto_uv2, imask)
         CALL iom_get  ( imask, jpdom_autoglo, 'resto', resto_uv2)
   ...
   SUBROUTINE dyn_dmp2( kt )
      !!----------------------------------------------------------------------
      !!                   ***  ROUTINE dyn_dmp2  *** (copy of dyn_dmp)
      !!                  
      ...
      !!----------------------------------------------------------------------
      !
      IF( ln_timing )   CALL timing_start( 'dyn_dmp2' )
      !
      !
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1  ! vector opt.
                  zua = resto_uv2(ji,jj,jk) * ( zuv_dta(ji,jj,jk,1) - ub(ji,jj,jk) )
                  zva = resto_uv2(ji,jj,jk) * ( zuv_dta(ji,jj,jk,2) - vb(ji,jj,jk) )
      ...
      !                zua = resto(ji,jj,jk) * ( zuv_dta(ji,jj,jk,1) - ub(ji,jj,jk) )
      !                zva = resto(ji,jj,jk) * ( zuv_dta(ji,jj,jk,2) - vb(ji,jj,jk) )
      !             ELSE
      !                zua = 0._wp
      !                zva = 0._wp  
      !             ENDIF
      ...
      !
      !
      IF( ln_timing )   CALL timing_stop( 'dyn_dmp2')
      !
   END SUBROUTINE dyn_dmp2
